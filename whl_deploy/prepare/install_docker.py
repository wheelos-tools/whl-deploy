...
class DockerManager:
    ...
    def _is_docker_already_functional(self) -> bool:
        ...
        try:
            ...
            info(
                "Docker not detected or not functional. Proceeding with installation."
            )
            return False
    ...
    def _install_prereq_packages(self) -> None:
        ...
        info(
            "Installing prerequisite packages (ca-certificates, curl, gnupg)..."
        )
        execute_command(
    ...
    def _setup_docker_repo_and_install(self) -> None:
        ...
        curl_result = execute_command(["curl", "-fsSL", gpg_url],
                                      use_sudo=False,
                                      text=False,
                                      check=True)
        execute_command(["gpg", "--dearmor", "-o",
                         str(DOCKER_GPG_KEY_PATH)],
                        use_sudo=True,
                        input_data=curl_result.stdout.decode('utf-8'),
                        check=True)
        execute_command(
            ["chmod", "a+r", str(DOCKER_GPG_KEY_PATH)],
            use_sudo=True,
            check=True)
    ...
    def _post_install_settings(self) -> None:
        ...
        if current_user:
            ...
            execute_command(["usermod", "-aG", "docker", current_user],
                            use_sudo=True,
                            check=True)
            info(f"User '{current_user}' added. "
                 r"Log out and log back in for changes to take effect.")
        else:
            warning(f"Cannot automatically add user '{current_user}' "
                    r"to 'docker' group.")
            warning(f"To use Docker without 'sudo', add your user manually: "
    ...
    def uninstall(self) -> int:
        ...
        try:
            ...
            try:
                ...
                error(f"Failed to remove Docker packages."
