...
class NvidiaToolkitManager:
    ...
    def _get_latest_toolkit_version(self) -> str:
        ...
        """
        info(
            "Attempting to determine latest NVIDIA Container Toolkit version from apt repositories..."
        )
        try:
    ...
    def _check_pre_conditions(self) -> None:
        ...
        if self.arch == "x86_64":
            ...
            # assume using jetson with built-in drivers
            # TODO(All): Consider adding a more robust check for aarch64
            # platforms in the future.
            info(
                "Skipping NVIDIA GPU driver check on non-x86_64 architecture (aarch64)."
            )
    ...
    def _is_nvidia_toolkit_already_functional(self) -> bool:
        ...
        try:
            ...
            if "nvidia" in docker_info_output:
                ...
                info(
                    "NVIDIA Container Toolkit package found, but Docker runtime is not configured. Attempting to configure."
                )
                try:
    ...
    def _install_prereq_packages(self) -> None:
        ...
        info(
            "Installing prerequisite packages (apt-transport-https, ca-certificates, curl, gnupg, lsb-release)..."
        )
        execute_command(
            [
    ...
    def _setup_nvidia_toolkit_repo_and_install(self) -> None:
        ...
        info(
            "Skipping optional experimental package configuration (as per standard practice)."
        )
